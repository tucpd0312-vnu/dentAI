...
export default function ReceptionistProfile({ user }: { user: AuthUser }) {
  ...
  return (
    <div className="min-h-full space-y-5 bg-slate-50 p-5 lg:p-7">
      ...
      <section id="work-files" className="rounded-2xl border border-gray-200 bg-white p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="font-serif text-lg font-semibold text-gray-900">Tệp phân công</h3><p className="mt-1 text-sm text-gray-500">Tải hoặc thay thế bảng Excel phân công ca trực và công việc tại quầy.</p></div><span className="inline-flex items-center gap-1.5 rounded-lg border border-gray-200 px-3 py-2 text-xs text-gray-500"><span className="material-symbols-outlined text-[16px]">description</span>Tích hợp tệp Excel tại trang quản trị</span></div></section>
    </div>
  );
}
