...
function TelemedicineNav({ role }: { role: DemoRole }) {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 shadow-sm backdrop-blur">
      <div className="mx-auto flex min-h-[56px] max-w-[1900px] items-center gap-4 px-4 2xl:px-8">
        <Link href="/dashboard/" className="inline-flex h-9 shrink-0 items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 text-sm font-semibold text-slate-700 transition hover:border-blue-200 hover:bg-blue-50 hover:text-blue-700">
        ...
        <div className="ml-auto flex shrink-0 items-center gap-2.5">
          ...
          <button onClick={handleLogout} className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-slate-200 px-3 text-xs font-semibold text-slate-600 hover:bg-red-50 hover:text-red-600"><Icon className="text-[18px]">logout</Icon><span className="hidden sm:inline">Đăng xuất</span></button>
        </div>
      </div>
    </header>
  );
...
function AlternateCalendarView({ view, role, appointments }: { view: CalendarView; role: DemoRole; appointments: Appointment[] }) {
  if (view === 'week') return (
  ...
  return (
    <article className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 pb-3">
        ...
        <div className="flex flex-wrap gap-2"><ActionButton icon="folder_shared" onClick={() => onAction(`Đã mở bệnh án của ${item.patient}.`)}>Xem bệnh án</ActionButton><ActionButton icon="videocam" primary onClick={() => onAction(`Đang chuẩn bị phòng khám ${item.room}…`)}>Vào phòng tư vấn</ActionButton></div>
      </div>
...
export default function TelemedicineDashboard() {
  ...
  const dayAppointments = selectedDay === 14 ? APPOINTMENTS : APPOINTMENTS.slice(0, selectedDay % 3 === 0 ? 3 : 2).map(item => ({ ...item, id: `${item.id.slice(0, -2)}${String(selectedDay).padStart(2, '0')}`, date: `${String(selectedDay).padStart(2, '0')}/09/2026` }));
  const matchesFilters = (item: Appointment) => {
    const query = patientFilter.trim().toLocaleLowerCase('vi');
    const matchesPatient = !query || item.patient.toLocaleLowerCase('vi').includes(query) || item.phone.includes(query);
    const matchesStatus = statusFilter === 'Tất cả trạng thái' || stateLabel[item.state] === statusFilter;
    const matchesRoleFilter = role === 'doctor'
      ? doctorFilter === 'Tất cả hình thức tư vấn' || item.mode === doctorFilter
      : doctorFilter === 'Tất cả bác sĩ' || (doctorFilter === 'Chưa gán bác sĩ' ? false : item.doctor === doctorFilter);
    return matchesPatient && matchesStatus && matchesRoleFilter;
  };
  const selectedAppointments = dayAppointments.filter(matchesFilters);
      ...
      <main className="mx-auto max-w-[1900px] space-y-4 px-4 py-5 2xl:px-8">
        <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div><div className="flex flex-wrap items-center gap-2.5"><h1 className="text-lg font-bold tracking-tight text-slate-900">{role === 'doctor' ? 'Lịch hẹn của tôi' : 'Quản lý lịch hẹn'}</h1><span className="inline-flex items-center gap-1.5 rounded-full border border-emerald-200 bg-emerald-50 px-2 py-0.5 text-[10px] font-bold text-emerald-700"><span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />Đã đồng bộ</span></div><p className="mt-1 text-xs text-slate-500">Phòng khám Răng Hàm Mặt – Nhà G6</p></div>
            {role === 'receptionist' && <div className="flex flex-wrap gap-2"><ActionButton icon="download" onClick={() => showNotice('Đã tạo bản xuất Excel/PDF demo.')}>Xuất dữ liệu</ActionButton><ActionButton icon="add" primary onClick={() => showNotice('Đã mở biểu mẫu tạo lịch hẹn mới.')}>Tạo lịch hẹn</ActionButton><ActionButton icon="save" primary onClick={() => showNotice('Đã lưu toàn bộ thay đổi lịch hẹn.')}>Lưu thay đổi</ActionButton></div>}
          </div>

          <div className="mt-4 grid items-end gap-3 border-t border-slate-100 pt-4 xl:grid-cols-[minmax(180px,1fr)_minmax(180px,1fr)_minmax(200px,1.2fr)_auto_auto]">
            ...
            <label className="block"><span className="mb-1.5 block text-[10px] font-black uppercase tracking-wide text-slate-400">Bệnh nhân</span><span className="relative block"><Icon className="absolute left-3 top-1/2 -translate-y-1/2 text-[18px] text-slate-400">search</Icon><input value={patientFilter} onChange={e => setPatientFilter(e.target.value)} placeholder="Tên hoặc số điện thoại" className="h-11 w-full rounded-xl border border-slate-200 bg-white pl-10 pr-3 text-sm font-medium text-slate-700 outline-none placeholder:text-slate-400 focus:border-blue-400" /></span></label>
        ...
        <div className="overflow-x-auto">{view === 'month' ? <MonthCalendar role={role} selectedDay={selectedDay} setSelectedDay={setSelectedDay} /> : <AlternateCalendarView view={view} role={role} appointments={filteredAppointments} />}</div>
        ...
        <section className="rounded-2xl border border-blue-200 bg-white p-4 shadow-sm">
          ...
          <div className="mt-3 space-y-2.5">{selectedAppointments.map(item => role === 'doctor' ? <DoctorAppointmentCard key={item.id} item={item} onAction={showNotice} /> : <ReceptionAppointmentCard key={item.id} item={item} onAction={showNotice} />)}{selectedAppointments.length === 0 && <p className="py-8 text-center text-sm text-slate-400">Không có lịch hẹn phù hợp với bộ lọc.</p>}</div>
        </section>
